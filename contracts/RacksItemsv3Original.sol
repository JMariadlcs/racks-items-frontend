...
import "./IRacksItemsOriginal.sol";
contract RacksItemsv3 is IRacksItemsOriginal , ERC1155, ERC1155Holder, AccessControl, VRFConsumerBaseV2 { // VRFv2SubscriptionManager
   

    /*
        Ultimos cambios:
        -Interfaz funciona
        -Los eventos y structs ahora estan en la interfaz, si no daba error,pero  es exactamente lo mismo
        -Funciones internas las he puesto todas abajo
        -Funcion _itemStillAvailable para comprobar que ese item lo sigue teniendo y que el approval sigue
        -Mapping s_marketInventory para saber cuantos items de cada tipo tienes en venta
        -El view items solo devuelve los que no estan dentro de ese mapping
        ...
        -Ocupa lo mismo
        -Reutiliza los tickets en vez de crear uno nuevo cada vez
        -Gastas menos gas al listar un ticket
        -Menos almacenamiento
        -Los 2 contratos testeados y funcionando
    */
  ...
  */
  function getCasePrice() public override view returns(uint256) {
    return casePrice;
  }
  ...
  */
  function supplyOfItem(uint256 tokenId) public override view returns(uint) {
  ...
  */
  function rarityOfItem(uint256 tokenId) public override view returns(uint256) {
  ...
  */
  function buyTicket(uint256 ticketId) public override {
    ...
    require(!s_hasTicket[msg.sender]);
    require(!s_hadTicket[msg.sender]);
  ...
  */
  function getMarketTicket(uint256 ticketId) public override view returns(caseTicket memory) {
  ...
  */
  ...
  function getUserTicket(address user) public override view returns(uint256 ticket, uint256 ownerOrSpender){
    ...
    uint256 ticket=0;
      ...
      : (s_hadTicket[user] && s_ticketIsLended[user])?3
  ...
  */
  function uri(uint256 tokenId) override(ERC1155, IRacksItemsOriginal) public view returns (string memory) {
  ...
  */
  ...
  /// @notice Assert that a certain item is still approved and owned by its owner
